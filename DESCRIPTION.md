...
- **CI ([`.github/workflows/ci.yml`](./.github/workflows/ci.yml))** — three jobs on every push/PR: all 14 health gates; the seven skill pytest suites (comfyui, docx, pdf, powerpoint, xlsx, regex-vs-llm-structured-text, sqlite-queries) with deps from [`test-requirements.txt`](./test-requirements.txt); and a dedicated `self-test-harnesses` job that executes every registered `*_verify.py` harness against real duckdb/polars/pyarrow/numpy installs (deps from [`selftest-requirements.txt`](./selftest-requirements.txt)) — so an upstream engine change breaks CI instead of quietly rotting the docs.
