---
description: Hermes Agent second brain — 206 skills across 23 categories, memories, cron configs.
---

# Hermes Skills Repository (Second Brain)

This repository is the **second brain** of its owner's Hermes Agent environment: a centralized collection of **206 verified, audit-passing skills**, persistent agent memories, and cronjob configuration — organized so that any agent can clone it and be productive in under a minute.

## Start here (cheapest → most thorough)
- **[SKILLS-INDEX.md](./SKILLS-INDEX.md)** — flat one-line-per-skill index of all 206 skills; `grep -i <term>` is the fastest way to find a capability.
...
- **[REFERENCES-INDEX.md](./REFERENCES-INDEX.md)** — flat index of all 381 reference docs living inside skills' `references/` dirs (nested subdirs included); `grep -i <term>` REFERENCES-INDEX.md finds verified API maps / gotchas tables by topic without knowing which skill owns them.
...
| Verify this repo's own health | `py tools/verify-all.py` — all 12 gates in one run (`python` is a Store stub on Windows) |
...
- **`verify-all.py`** — **the one command**: runs all 12 gates (audit incl. the zero-threshold hardcoded-secret scan over every skill script + SKILL.md, links, index drift x5: SKILLS/CODE/REFERENCES/DEPENDENCY/.claude-plugin, cron validators x2, doc-count consistency + its mutation self-test + the secret gate's own mutation self-test) and prints a pass/fail table. Run before every commit.
...
- **CI ([`.github/workflows/ci.yml`](./.github/workflows/ci.yml))** — runs all 12 gates plus the seven skill pytest suites (comfyui, docx, pdf, powerpoint, xlsx, regex-vs-llm-structured-text, sqlite-queries) on every push/PR; test deps from [`test-requirements.txt`](./test-requirements.txt) (single source of truth — update it when adding suite dependencies).
