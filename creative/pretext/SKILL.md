...
metadata:
  hermes:
    ...
    related_skills: [p5js, claude-design, excalidraw, architecture-diagram, ascii-art, ascii-video]
...
- Image → text (load `skill_view(name='ascii-art')` / `skill_view(name='ascii-video')`)
- Pure canvas generative art with no text role — load `skill_view(name='p5js')`
