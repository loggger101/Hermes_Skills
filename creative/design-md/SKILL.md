...
For purely visual inspiration or layout examples, load `skill_view(name='popular-web-designs')`
instead. For *process and taste* when designing a one-off HTML artifact
from scratch (prototype, deck, landing page, component lab), load
`skill_view(name='claude-design')`. This skill is for the *formal spec file* itself.
