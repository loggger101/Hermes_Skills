...
Gives any machine with an SSH client (Linux, macOS, Windows 10+) zero-install access to a public internet URL for a local service — no ngrok/cloudflared binary, no account on the free tier. Covers starting/stopping/parsing tunnels end-to-end via `terminal`, plus four ready recipes: webhook capture server, MCP-over-HTTP exposure, sharing a local LLM endpoint (Ollama/vLLM/llama.cpp), and one-shot password-gated dev-server sharing.
