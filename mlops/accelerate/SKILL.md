...
Run PyTorch training across GPUs with minimal changes: the 4-line `Accelerator` conversion of any plain script, plus reference material for multi-GPU/multi-node launch, mixed precision (FP16/BF16), DeepSpeed/FSDP integration, custom plugins, and performance tuning.

## When to Use

- Adding distributed training to an existing PyTorch script without rewriting it
- Choosing between DDP / FSDP / DeepSpeed for a given model+hardware combo
- Mixed precision setup (FP16 vs BF16), multi-GPU or multi-node launch commands
- Debugging accelerate-specific behavior: device placement, gradient accumulation, save/load across ranks
