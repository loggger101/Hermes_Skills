...
**Round-6e (same day): upstream sweep continued → 183 skills.** Two more general-purpose ports from the unmined upstream optional-skills set, both verified against live content first: `devops/pinggy-tunnel` (zero-install public URLs for local services via SSH reverse tunnels to Pinggy — free tier needs no account; covers start/parse/verify/teardown over terminal, access-control username keywords b:/k:/w:/co/x:, and four self-contained recipes: webhook capture server, MCP-over-HTTP with bearer gating, sharing a local Ollama/vLLM/llama.cpp endpoint, one-shot password-gated dev-server share) and `mcp/mcporter` (npx-based terminal MCP client — list configured/ad-hoc servers, inspect tool schemas, call tools in key=value/function/JSON syntax with --output json, OAuth auth, daemon mode, CLI-wrapper + TS type generation; cross-linked to the existing server-side skill fastmcp). Both keep upstream frontmatter style (single-quoted descriptions are fine — audit only enforces length), gained the two required body sections, and were copied from on-disk sources with sha256 verification. Also: switched multi-point count updates to a verification script that asserts each anchor occurs exactly once before substituting — caught one line my first pass missed (the "across N skills" xref claim). Final state at 6e commit: **183 skills / 23 categories / 425 xrefs**, all 9 gates green.

**Round-6f (same day): upstream sweep concluded → 184 skills.** `web-development/publish-site` — versioned static-site deploys on user-owned infrastructure (GitHub Pages default, Cloudflare/Netlify ladder), with the discipline baked in: local preview + optional shareable tunnel for sign-off, git tag before every deploy (`deploy-YYYYMMDD-HHMM`), live-URL verification via curl (never trust the deploy log alone), one-command rollback. Chosen over `mlops/research/dspy` (594 lines; requires pip dspy + OpenAI/Anthropic API keys — this brain runs local LM Studio, so it would be dead weight) and `research/darwinian-evolver` (AGPL upstream tool invoked via subprocess, 50–500 LLM calls per run). This closes the upstream optional-skills sweep: of 140 upstream skills, 32 are now in this brain; every remaining unmined one is niche for these domains (blockchain/gaming/payments/finance models/health/MLOps frameworks/security infra) and is documented as such — future cycles should re-scan only if the user adds new stars or a domain need appears. Also verified: `github-pr-workflow/references/git-workflow-recipes.md` was ALREADY distilled from tiimgreen/github-cheat-sheet in an earlier round (fixup/autosquash, PR checkout, revert, Windows notes), so that star is fully mined; and the full local↔repo inventory diff came back CLEAN at 183 skills — 0 byte mismatches across all common skill dirs, only known local-only items (rss-feeds, reddit-reading) plus gitignored __pycache__/.pytest_cache artifacts. Final state: **184 skills / 23 categories / 425 xrefs**, all 9 gates green.

**Round-6g (same day): ECC sweep concluded + knowledge-layer additions → 299 reference docs.** Ran a keyword-scored domain sweep over the remaining 271 unassessed ECC skills; top scorers were `git-workflow`/`autonomous-loops`/`dmux-workflows` (all overlap this brain's github cluster), `django-*` + `flutter-dart-code-review` (frameworks not in use here), and `github-ops`. From `github-ops`, the ONE piece of unique value — a concrete **threat model for gh CLI output** (issue/PR/review/commit/CI-log content is untrusted data, never instructions; repo content cannot authorize writes; fork repro steps are an attack vector) plus a numeric stale-item policy and triage label taxonomy — was distilled to `github/github-issues/references/untrusted-repo-content.md` with the rest rejected as overlap. Also added `software-development/hermes-agent-skill-authoring/references/skill-evolution-pipeline.md`: verified-at-source documentation of NousResearch/hermes-agent-self-evolution (DSPy+GEPA; CLI flags read from evolve_skill.py — --skill/--iterations/--eval-source synthetic|golden|sessiondb/--optimizer-model openai/gpt-4.1 default/--run-tests constraint gate; ~$2–10 per run on hosted models, LiteLLM-style names so a local LM Studio endpoint is possible but reflection quality unverified) as the "measurably underperforming skill" path in that skill's editing workflow — evolution proposes text, verify-all + test suites still gate. REFERENCES-INDEX regenerated 296→298 (caught: an earlier doc add had skipped regeneration) →299 with this batch; DESCRIPTION count line reconciled each time. Final state: **184 skills / 23 categories / 425 xrefs / 299 reference docs**, all 9 gates green, local↔repo parity clean.

**Round-7a (same day): cross-link enrichment — isolated nodes 17 → 1.** Quality pass found 16 of the 17 no-outbound `related_skills` skills were recent ports or hub installs that never got wired into the network; all link targets verified to exist in-repo first (check_candidates.py), then a scripted edit replaced/inserted one frontmatter line per skill on BOTH live and repo copies with re-read verification of each: one-three-one-rule→{decision-questionnaire, grilling-interview}; full-output-enforcement→{design-taste-frontend, redesign-existing-projects}; bit-identity-float-pipelines→{build-systems-data, verification-culture}; jupyter-notebook→python-data-science; space-data-pipelines→{astro-toolkit-selection, orbital-mechanics-data}; pinggy-tunnel→ssh-remote; watchers→{blogwatcher, cron-job-authoring}; fastmcp↔mcporter (mutual); evaluating-llms-harness→weights-and-biases; bioinformatics→{gget, pubmed-database}; scrapling→blocked-page-recovery; oss-forensics→semgrep-rule-creator; software-development/github→{github-pr-workflow, github-issues}; har-derived-api-client→rest-graphql-debug; publish-site→{static-site-patterns, static-site-seo}. `xurl` deliberately left standalone: its only natural partner (reddit-reading) is local-only and not in the repo — a ref to it would break the related_skills-resolution gate. DEPENDENCY.md regenerated: 425 → **449 xrefs**, hubs 73→96, isolated nodes 14→**1**; README's two stale count claims (overview line + Live Invariants "393 across 167" — a leftover from the pre-round-6 pass) reconciled to generator output. Also closed out `repowise-dev/repowise` (6,387★): AGPL-3.0 codebase-intelligence MCP tool → rejected per standing policy (AGPL subprocess tools), no content distilled since it is a runtime product rather than knowledge. Final state: **184 skills / 23 categories / 449 xrefs / 299 reference docs**, all 9 gates green, byte-parity verified on every edited skill.

**Round-7b (same day): first new test suite since CI runner shipped — `devops/sqlite-queries` gains `tests/`.** Test coverage was the next measurable gap: only 6 of 184 skills had suites while `tools/run-skill-tests.py` already discovers any `<skill>/tests/` automatically (zero workflow edits needed). Chose sqlite-queries as the first addition because it is pure stdlib (`sqlite3`) — zero new deps in test-requirements.txt, and every claim in SKILL.md is mechanically checkable. 20 tests / conftest fixture DB (users/orders with FKs + index) asserting documented behavior: schema discovery via sqlite_master vs table_info, the "COUNT(*) from sqlite_master is wrong" pitfall, row-count UNION pattern, FK cascade/RESTRICT semantics, EXPLAIN QUERY PLAN shows index usage on the indexed column and not on unindexed ones, transaction rollback restores pre-transaction state exactly, VACUUM INTO backup passes integrity_check + holds identical query results, CSV export round-trips with headers, JSON export via json_group_array/json_object (labels must be single-quoted — double quotes parse as identifiers), .import pitfall: header row becomes column names and non-numeric data lands all-TEXT. Four tests exercise the real `sqlite3` CLI (.tables/.schema/.headers dot-commands + .mode csv import) under skipif(no binary): they SKIP on this Windows host (no sqlite3.exe installed — verified) but RUN in CI where ubuntu-latest ships it, so both environments stay honest rather than one silently covering less. Two failures caught by actually running: my first JSON test used double-quoted json_object labels (SQLite treats them as identifiers → error), and a NUMERIC-affinity coercion assertion that Python parameter binding does not trigger the way raw SQL does — both corrected to assert what SQLite ACTUALLY does, keeping each test a check of documented skill claims rather than an incidental behavior. SKILL.md gained a "Regression tests" pointer (matching docx/pdf/powerpoint/xlsx convention); README CI section updated from stale "five shipped suites" to the live 7-suite discovery model with per-suite counts (comfyui 117 / docx 29 / pdf 21 / powerpoint 21 / xlsx 12 / regex-vs-llm 14 / sqlite 20 — counted from disk, not remembered). CODE-INDEX regenerated (126 code files now; the new gate caught the drift before commit). Local↔repo byte-parity verified on SKILL.md + both test files. All 9 gates green at commit time.


**Round-7b fixes (next day): two real bugs caught by actually running the suite — and one CI-only bug they exposed.** (1) The first sqlite push (`7ce47bd`) FAILED in CI: `test_cli_import_creates_table_from_header` passed dot-commands as ONE argv string with an embedded newline (`".mode csv\n.import ..."`), which the shell rejects ("bad argument") on every platform — reproduced locally by downloading the official sqlite-tools-win-x64 zip (3.53.4) into Temp and running all 20 tests against a real binary: fixed to separate argv entries, verified green with CLI present AND skip-mode without it. Lesson: CI is the only place those 4 tests execute — never trust them unrun; get a local binary before shipping CLI-dependent tests. (2) `test_cli_header_csv_flag_shape` broke on Windows CRLF line endings from the sqlite3 shell (`splitlines()` yields spurious blank lines between rows); fixed with per-line strip + drop-empty normalization so one test is platform-stable across LF/CI and CRLF/local. (3) ROOT CAUSE of all three consecutive CI failures at `7ce47bd`/`0cfc5b4`/`1dfb9b6`: the health-gates job failed on **drift: DEPENDENCY** — `regen-dependency-map.py` embedded "*Last generated: <today>*" into DEPENDENCY.md, so regeneration after UTC midnight always produced a different byte string than the committed one (local gates passed because local date == commit date; CI ran in ET-morning = previous UTC day). This bug was latent since DEPENDENCY.md existed and would have bitten ANY overnight-window push. Fix: removed the generation-date stamp from BOTH generators (`tools/regen-dependency-map.py` + the built-in copy inside `sync-hermes-skills.py`) — content is fully determined by frontmatter, git history records when each version was written; DEPENDENCY.md regenerated without the line and all 9 gates green. Also hardened `run-skill-tests.py`: on failure it now prints each FAILED test name (capped at 25) instead of a bare "1 failed" — that blind spot is what cost an extra CI round-trip to find which test broke. Process lesson: when a gate passes locally but fails in CI, diff the two environments' DATES first for any generator with embedded timestamps; and never ship CLI/subprocess tests without running them against a real binary at least once.

**Round-19c (2026-09-14): star-list coverage ledger — all 51 stars now have an evaluation record.** A mention-count sweep of every starred repo slug + bare name across the brain's markdown found **7 stars with zero documented evaluation**: free-for-dev, public-apis, awesome-creative-coding, awesome-design, TheAlgorithms/Java (plus spellbook-of-modern-webdev and ionic-framework, previously skipped without a written record). All seven were cloned fresh (`--depth 1`) and inspected at source level before being recorded:

| Star | Verdict (verified from clone) |
|---|---|
| `ripienaar/free-for-dev` | Link-list of free-tier dev services (~2,300 entries in one HTML page). No code. Its AGENTS.md explicitly rejects AI-edited contributions — do not propose PRs there. **Rejected: no computational content.** |
| `public-apis/public-apis` | 2,296-line README table of free public APIs (Auth/HTTPS/CORS columns) + a ~1,200-line validation toolchain (`scripts/validate/{format,links}.py`). The validator is a generic markdown-table/link checker — this brain's `tools/check-links.py` covers the same ground more generally. **Rejected: link-list; no unique pattern beyond check-links.** |
| `terkelg/awesome-creative-coding` | Curated beginner resource list (books/courses/tools for creative coding). No code of its own. Overlaps the existing creative cluster's coverage surface. **Rejected: link-list, overlap.** |
| `gztchan/awesome-design` | UI/UX resource links (stock/icons/color/toolkit), Travis-CI era badges — partially stale by nature. Brain already carries design-system knowledge in 5+ dedicated skills (stitch, popular-web-designs, taste presets). **Rejected: link-list, overlap + staleness risk.** |
| `TheAlgorithms/Java` | Same catalog structure as TheAlgorithms/Python (sorts/graph/dp/maths/...), Java implementations. Brain is Python-first; the Python twin was mined to exhaustion in round 18 (`data-science/algorithms-python-catalog`) with oracle-verified code — a parallel Java catalog would duplicate it without new knowledge. **Rejected: language-duplicate of an already-mined star.** |
| `dexteryy/spellbook-of-modern-webdev` | "Big picture taxonomy" of modern JS web dev = curated link list (35 files, one README + 2 images + a bookmark script). Brain's static-site-patterns + popular-web-designs cover the actionable surface. **Rejected: link-list** (recorded here; previously skipped without documentation in round 19). |
