...
A comprehensive collection of **173 Hermes Agent skills** across 23 categories — the second brain for its owner's Hermes Agent environment. See [DESCRIPTION.md](./DESCRIPTION.md) for the task-to-skill quick table and lookup ladder.
...
This repository serves as a centralized database of all **173 Hermes Agent skills**, organized by category. Skills are reusable procedures and workflows that extend Hermes Agent's capabilities. All skills follow the standard `SKILL.md` format with consistent frontmatter, section headers, and `related_skills` cross-references (406 cross-references mapped across 172 skills, 10 standalone). See [audit notes](docs/archive/audit-notes-skills-repo-pass.md) for the full audit details and [DEPENDENCY.md](./DEPENDENCY.md) for the full relationship map.
...
| [devops/](./devops/) | Infrastructure, containers, and deployment | 7 |
...
| [research/](./research/) | Research and content discovery | 12 |
...
| [software-development/](./software-development/) | Development tools and workflows | 45 |
| [web-development/](./web-development/) | Web/API client derivation (HAR-based) | 2 |

**Total: 173 skills across 23 categories** — counts below are regenerated from live frontmatter; keep them in sync via `python tools/gen-skills-index.py` and the audit.

### Skill Catalog

All 173 skills organized by category:
...
3. **Research-harvest ports** — added across successive starred-repo deep-dive rounds (145 → 162 → 166 → 167 → 172 → 173), including hub installs (`hermes skills install official/...`) and MIT-licensed external ports with their licenses carried in frontmatter.
...
| [`audit-skills.py`](./tools/audit-skills.py) | Validates all 173 skills: YAML frontmatter, description length, `related_skills` resolution, body section presence, `skill_view()` call sync, category `DESCRIPTION.md` checks | Defined for Sun 3 AM in `skill-audit.json` — **not registered**; run manually |
