...
<!-- source: sentrux/sentrux (MIT) — metrics/arch/ + dsm + testgap modules read at source level 2026-09-11; implementation written and verified against synthetic fixtures + economicspace -->

## What This Skill Does

Computes **architecture-level** metrics for a Python project from its import
graph — the layer above `code-quality-signal`'s root-cause scores. The script is
stdlib-only (`ast`, no deps), runs in seconds (3s on 27-file repos, ~5s on this
190-script repo).
...
## Verified Behavior (2026-09-11)

| Project | Result | Notes |
|---|---|---|
| synthetic fixture (cycle pair, 3-deep chain, god file candidate, ABC in core) | max_level=3 ✓; cycle svc_a↔svc_b reported as the only violations ✓; blast radius c2=4 (bottom of chain hits most dependents) ✓; `core` flagged stable foundation + excluded from D average ✓; DSM inversions=0, correct(below)=17 ✓ | every property hand-verified against fixture construction |
| economicspace (27 files, real imports) | 3.0s; master blast radius=4; test gaps led by calc cc=119 — plausible for a branch-heavy pipeline module | |
