...
- `references/monolith-decomposition-pipeline.md` — the 5-pattern analysis pipeline that RUNS BEFORE any of the above (leaf-node component sizing heuristics, domain-vs-infrastructure dedup split, orphaned classes, coupling as strength×distance×volatility with the 8-cell diagnosis table per Khononov) + co-change graph noise-filtering for ownership clustering.
- `references/modular-design-principles-violations-and-split-criteria.md` — the principle layer: ten modularity principles, eight NAMED boundary violations (reach-through persistence, leaky exports, centralized data ownership...) as review-ready labels for what the metrics point at, and the six split signals + merge-or-wait rule.
