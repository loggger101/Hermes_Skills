...
## 6b. tlc-spec-lean v1.0.0: freeze the obligations, not the plan (mined 2026-09-17)

The leaner sibling — one human-reviewed document carrying EARS criteria + path + entities + interface, then proof-backed checks, build, independent Verifier; NO task breakdown at all. Two principles that generalize: **granularity is not quality** (fifteen one-file tasks buy ordering, not correctness — proof coverage buys correctness), and **a plan the model must obey competes with the obligations for attention** (Where/Tools/Depends-on are the builder's job to decide, so they aren't written down).

- **One-way doors**: only hard-to-reverse decisions get a row in the frozen document, each with its LITERAL shape; everything reversible is decided while building and reviewed in the diff. Irreversibility — not "importance" — is what earns a pin.
- **Pinned rules** (hold even if no reference file is read): every check = one observable claim + concrete value + proof whose exit code settles it ("no proof, no check"); tests assert what checks say, never what the code happens to do; never weaken/delete/skip an assertion — a genuinely wrong check is stop-and-ask, not an edit; approved rows are additive-only (a door discovered while building gets its row BEFORE the code that closes it); Verifier = fresh sub-agent dispatched by whoever holds the whole feature, NEVER by a builder, over `<feature base>..HEAD` with every check ("never optional, never prompted, never the author"); verification profile declared in AGENTS.md and named in the report (so "no faults injected" reads as deliberate, not forgotten); **completion gate is a script** (`validate_verification.py` exit 0), not a feeling; blast radius — an approved spec authorizes local edits/commits only: push/deploy/prod-data each need explicit per-action go-ahead.
