...
metadata:
  hermes:
    ...
    related_skills: [requesting-code-review, mattpocock-subagent-driven-development, test-driven-development]
---
...
rounds until every branch is resolved and nothing is silently assumed. Combines the phase discipline of the original with the frontier-rounds mechanic from mattpocock/skills' `grilling`.

## What This Skill Does

Runs a structured adversarial interview over your plan: models it as a design tree (each decision branches into the decisions hanging off it), asks one question at a time in rounds, forces explicit choices on ambiguous branches, and ends with a resolution summary of every answered branch — so nothing is silently assumed before implementation starts.
