...
description: "Docs governance: constitution, map, status, history roles."
version: v0.1.0
...
metadata:
  hermes:
    tags: [documentation-governance, docs-drift, agents-md, adr, maintain-phase]
    related_skills: [codebase-onboarding, mattpocock-domain-modeling]
---
...
Assigns four non-overlapping roles — constitution, map, status, history — to a project's **existing** documentation, links those roles from the active agent harness (AGENTS.md / CLAUDE.md), and defines small update rules that keep the sources useful. The roles matter; the filenames do not. This is a maintain-phase practice: for one-time exploration of an unfamiliar repository, use `codebase-onboarding` first.
...
Do **not** use this for a throwaway script, and do not create a parallel documentation system when the repository already has one.
