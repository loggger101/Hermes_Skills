...
3. Hunt candidates in these categories:
   ...
   - **Automated checks** — could linting/typing/tests/filesystem-linters have caught an error that actually happened? Read the repo's own check commands FIRST (its build-tool `lint`/`check` scripts, its CI workflow): a check that already exists but sits unwired or silently broken is itself the finding. A repo with no guardrail at all (no pre-commit hook and no CI job running lint/typecheck/test) is also a standing missed opportunity — propose wiring one. *Use when* the agent made a mistake a check would catch, or the repo has no guardrail.
   - **Coding standards** — should the reviewer get a new rule (or lose/clarify one)? Classify the violation first: a **mechanical** one (fixed syntactic pattern, banned API, import shape, file-location rule) gets a deterministic check full stop — custom linter rule, pre-commit hook, or CI job, whichever is cheapest in this repo; default to *building the check* over writing the rule. Reserve standards docs for genuine **judgement calls** (cross-file consistency, "matches surrounding style"). *Use when* review failed to catch something.
