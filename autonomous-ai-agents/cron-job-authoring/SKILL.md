...
- **Forgetting to update model references across all config files.** When a cron job's model changes (e.g. from `claude-sonnet-4-20250514` to `qwen/qwen3.6-35b`), the change must be applied to ALL files that reference the model: the cron JSON config (`model`, `provider`), the README tables that list the config, AND any guardrail text strings that mention the model pin. Missing one location causes drift — the guardrails text will claim one model while the actual config uses another, confusing future debugging. Always grep for the old model string after updating.
...
- **Incorrect path resolution in invoked scripts.** When a cron job invokes a Python script via `terminal`, the script must compute `REPO_ROOT` from `__file__`. The `parents[N]` index depends on how deep the script lives: a script at `repo/tools/script.py` needs `parents[1]` (repo root), NOT `parents[2]` (which resolves to the parent of the repo). A wrong index causes the script to scan unrelated directories or produce false errors. Always verify before committing: `python -c "from pathlib import Path; print(Path('tools/script.py').resolve().parents[1])"`. See `references/script-path-resolution.md`.
- **Scripts that rglob() the repo root must exclude their own output directories.** A sync or audit script that does `root.rglob("SKILL.md")` will find files in `profiles-export/` and `memories-export/` — generated copies from the sync script's export phase — and report them as duplicate skills or broken refs. Always add exclusion checks for `.git/`, `.hermes/`, `profiles-export/`, and `memories-export/` in any recursive file scan. See `references/script-path-resolution.md`. On Windows, normalize path separators with `.replace("\\", "/")` before checking for these substrings, or the exclusion check will silently fail (paths use backslashes on Windows, so `"profiles-export/" in "C:\\repo\\profiles-export\\skill\\SKILL.md"` is `False`).

- **Script dry-run flags that don't actually skip file operations.** When a cron job invokes a script with `--dry-run`, the script must not mutate any files — neither in the repo nor in the local Hermes environment. A common bug is that `--dry-run` only skips git operations (pull/push) but still copies files in the sync phase, so a "preview" run is silently destructive. After running `--dry-run`, verify no export directories (`profiles-export/`, `memories-export/`) were created and no skill files were modified in either direction. See `references/script-path-resolution.md`.
- **Python3 not available on Windows for no_agent subprocess calls.** When a no_agent cron job's script invokes `subprocess.run(["python3", ...])`, it may fail on Windows where `python3` resolves to the broken Microsoft Store stub (exit code 49). Use `shutil.which("python")` first, then fall back to `python3`:
...
This is especially important for scripts that invoke `audit-skills.py` or other Python helpers as subprocesses — a failed subprocess causes the cron job to report `success=False` even when everything else is correct.
...
6. If the job invokes a Python script via `terminal`, verify the script's `REPO_ROOT` resolves to the repo root (not a parent directory) — e.g. `python -c "from pathlib import Path; print(Path('tools/script.py').resolve().parents[1])"` should print the repo path.
7. If the script runs with `no_agent=true` and invokes Python subprocesses, verify it uses `shutil.which("python")` (not hardcoded `python3`) — on Windows, `python3` resolves to a broken Store stub that causes silent failures.
8. If the script supports `--dry-run`, verify a dry-run invocation produces no file changes (no export directories created, no skill files modified in either direction).
