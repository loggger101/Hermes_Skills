...
7. **The sync script's commit gate is driven by in-memory counters, not git status.** When verify-all fails after the script has already copied files and regenerated indexes, nothing is committed. Re-running the sync script sees `total_changes=0` (all files are already on disk from the prior run, so push_skills and mem_result are empty) and stays silent — the orphaned uncommitted changes remain undelivered. After fixing gate failures, commit manually: `git add -A && git commit -m "chore: sync ..." && git push`, using the `hermes-cronbot <cronbot@hermes.local>` identity.

8. **Category DESCRIPTION.md files are stale in local after sync.** The sync script regenerates these (via `gen-skills-index.py`) in the repo **after** the repo→local pull step has already completed, so local copies never receive the update. After a sync, check `git status` for modified `*/DESCRIPTION.md` files and copy them to local manually, or re-run the sync — the pull step picks up regenerated DESCRIPTIONs on the second pass.

9. **Verify-all failures need targeted fixes matched to the failure class, not sync re-runs.** Re-running the sync script without addressing the root cause fails identically — it is a gate, not a retry. Match the failure to its fix, then re-run `python3 tools/verify-all.py` to confirm: `broken_refs` → fix `related_skills` entries (use the skill `name` field, not a repo-relative path); `check-links` → replace non-standard link protocols (e.g. `skill://`) with relative markdown paths (`./../category/skill/SKILL.md`); `doc counts` → update numbers in README.md/DESCRIPTION.md to match the live counts in SKILLS-INDEX.md, DEPENDENCY.md's Network stats line, `.claude-plugin/plugin.json`, and REFERENCES-INDEX.md; `drift: DEPENDENCY` → re-run `tools/regen-dependency-map.py`.
