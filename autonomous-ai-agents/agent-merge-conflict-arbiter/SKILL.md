...
Takes two conflicting agent branches (diffs + each side's stated intent) and produces an impartial merged resolution: classify the conflict type, preserve BOTH sides' intent where compatible, pick one side with justification where they truly collide, verify the result builds/tests green, and report every decision so both agents can audit it.
