...
Turns any machine with Node.js into an MCP client: list already-configured or ad-hoc MCP servers, inspect their tool schemas, call tools with `key=value`/function/JSON syntax (machine-readable via `--output json`), handle OAuth auth, run a persistent daemon, and generate CLI wrappers / TypeScript clients for any server. No config needed for one-off calls against an HTTP URL or a stdio command.

## When to Use

- You need to call tools on an MCP server from the terminal (or verify what servers/tools are configured on this machine) without writing client code — `npx mcporter list` / `mcporter call <server.tool> ...`.
- Testing a remote HTTP/SSE or stdio MCP server ad-hoc before wiring it into Hermes config.
- Generating a reusable CLI wrapper (`generate-cli`) or TypeScript types/client (`emit-ts`) for an existing MCP server.
- Building the server side instead → `fastmcp` (authoring, testing, and deploying Python MCP servers).
