---
name: security-audit
description: "Source-first six-phase codebase security audit workflow."
version: 1.0.0
author: Cloudflare (https://github.com/cloudflare/security-audit-skill), ported by Hermes Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [security, audit, vulnerability, review, pen-test, threat-modeling, findings-report]
    category: security
...
Upstream source: cloudflare/security-audit-skill @ c1c8a8c (main), MIT. The skill that
seeded Cloudflare's vulnerability-discovery harness — this is the single-repo starting
point it evolved from. Ported verbatim except frontmatter, the two sections below, and
link paths adapted to this repo's layout (`references/`, `scripts/`).

## What This Skill Does

Turns an agent into a source-first security auditor: six phases (reconnaissance ->
coverage-led hunting waves with critic passes -> fresh-verifier candidate validation ->
schema-validated structured findings -> independent record verification -> target-neutral
report). Findings carry three distinct verdicts — `confirmed` (complete source trace +
bounded observed result, the only one that gets severity), `needs_validation` (a specific
source-grounded boundary hypothesis blocked by an exact missing fact; never a low-
confidence confirmed vuln, and it has no severity), and `rejected`. A deterministic
coverage ledger makes spend countable and multi-run additive: prior runs seed revalidation
units instead of implying "rest is fine."

## When to Use

Explicit codebase audits or pen-test requests, full/comprehensive/end-to-end security
reviews, or when the user wants report artifacts. For focused questions, triage, a single
suspicious path, or methodology advice, use guidance mode — load only the relevant phase
or companion doc; do not run all six phases or create audit output directories. Pair with
`mattpocock-security-review` for per-language review checklists and `oss-forensics` when
the target is a suspected malicious package rather than an own-repo defect hunt.

## Verified on This Machine (2026-09-17, port pass)

Node v22.23.2: the shipped test suites run via `node --test scripts/validate-findings.test.cjs
scripts/validate-coverage-ledger.test.cjs`. Core validation logic is cross-platform and
passes; **on Windows, 7 CLI-subprocess tests fail by design** — the validator's input-read
path requires POSIX no-follow/nonblocking open flags (`O_NOFOLLOW`, `O_NONBLOCK`) for its
"OS-level input protection," which do not exist on Win32, so it fails closed with
`Failed to read findings JSON: OS no-follow and nonblocking input protection is unavailable`.
That is the correct fail-closed behavior (it refuses to run without the protections), but
on this host use the validators' exported functions (`validateDocument`, `collect`) from a
Node script instead of the CLI, or run the full suite on Linux/macOS.
