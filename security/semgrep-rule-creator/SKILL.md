...
description: "Create tested Semgrep rules with taint-mode support."
version: v1.0.0
author: Hermes Agent (ported from trailofbits/skills semgrep-rule-creator)
license: CC-BY-SA-4.0
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [semgrep, static-analysis, security, rules, taint-mode]
    related_skills: [mattpocock-security-review, requesting-code-review]
---

<!-- source: trailofbits/skills plugins/semgrep-rule-creator (starred-repo deep-dive 2026-09-05). Upstream license CC-BY-SA 4.0 — attribution retained per that license; references/ copied verbatim from upstream. -->

## What This Skill Does

Creates production-quality Semgrep rules with a mandatory test-first workflow: analyze the
problem, write tests (vulnerable AND safe cases), inspect the AST, write the rule, iterate until
100% of tests pass via `semgrep --test`, then optimize. Prioritizes taint mode for data-flow
vulnerabilities (untrusted input reaching a dangerous sink) over plain pattern matching — it only
alerts when untrusted data actually reaches the sink, dramatically cutting false positives.

## When to Use

- Writing Semgrep rules for specific bug patterns or security vulnerabilities in your codebase
- Building custom static-analysis detections / enforcing coding standards as rules
- NOT for running existing rulesets (use a plain `semgrep --config` run)
