...
Indexes two open-source bioinformatics skill libraries — **bioSkills** (385 reference skills: code patterns, parameter guides, decision trees) and **ClawBio** (33 runnable pipeline skills with reproducibility bundles) — and fetches the specific one you need on demand. Instead of bundling hundreds of domain-specific skills into this brain, it keeps a single gateway plus a full index by domain, so any genomics/computational-biology question resolves to an expert-validated reference in seconds.

## When to Use

- Any task touching sequencing data (FASTQ/BAM/VCF), RNA-seq (bulk or single-cell), spatial transcriptomics, epigenomics (ChIP/ATAC/methylation/Hi-C), variant calling & clinical annotation, pharmacogenomics, GWAS/population genetics, metagenomics/microbiome, genome assembly, structural biology, proteomics, pathway analysis, immunoinformatics, CRISPR screens, or omics machine learning.
- When you need correct tool flags/parameters (GATK, BWA, STAR, Salmon/kallisto, DESeq2, edgeR, Seurat, Scanpy, PLINK, Samtools/Bcftools) without guessing from memory — fetch the matching bioSkills reference first.
- When a task needs an executable pipeline with a reproducibility bundle (report.md + commands.sh + environment.yml) rather than just guidance — use a ClawBio skill instead.
