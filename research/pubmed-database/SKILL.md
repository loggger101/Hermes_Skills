...
description: "PubMed/NCBI E-utilities: MeSH queries, PMID lookup, API."
version: v0.1.0
...
metadata:
  hermes:
    tags: [pubmed, ncbi-eutilities, mesh-terms, biomedical-literature, systematic-review]
    related_skills: [literature-review, arxiv, grounded-citations]
---
...
- Running systematic-review search passes that need repeatable search strings (pairs with the `literature-review` workflow).
- Using NCBI E-utilities directly from Python, shell, or another HTTP client.

## What This Skill Does

Direct PubMed / NCBI E-utilities search: build repeatable MeSH + field-tag queries, run esearch/esummary/efetch/elink from Python or the shell with correct rate limiting and environment-based API keys, and log every search pass so results are reproducible. The biomedical counterpart to `arxiv` (which covers CS/math/physics preprints).
...
variables, never in committed files or command history. Without a key the rate
limit is 3 requests/second (10 with one) — the sleep below respects that.
...
def esearch(query: str, retmax: int = 20) -> list[str]:
    ...
    time.sleep(0.35)  # stay under the unkeyed 3 req/s limit
...
- Do HTTP calls handle non-200 responses before parsing (e.g. `raise_for_status()` with requests)?
