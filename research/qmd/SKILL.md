...
Hybrid local search (BM25 + vector) over personal notes, docs, and transcripts via the `qmd` CLI — no cloud, sub-second recall across a whole knowledge base. Requires Node.js >= 22; covers install, indexing a directory, querying with filters, and wiring results into agent workflows.
